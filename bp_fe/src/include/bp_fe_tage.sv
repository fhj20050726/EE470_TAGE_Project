 ...
 * bp_fe_tage.sv
 *
 * Verilator-safe TAGE conditional branch direction predictor.
...
module bp_fe_tage
 ...
 #(parameter bp_params_e bp_params_p = e_bp_default_cfg
  ...
  initial begin
    ...
    for (init_t = 0; init_t < tage_tables_lp; init_t = init_t + 1) begin
      for (init_i = 0; init_i < tage_els_lp; init_i = init_i + 1) begin
        ...
        tage_ctr   [init_t][init_i] = 3'b011;
        tage_useful[init_t][init_i] = 2'b00;
      end
    end
  end
  ...
  logic pred_n, alt_pred_n;
  logic provider_found_n, alt_found_n;
  logic [provider_width_lp-1:0] provider_n;
  ...
  logic [provider_width_lp-1:0] update_provider;
  ...
  logic alloc_v;
  logic [provider_width_lp-1:0] alloc_table;
  logic [tage_idx_width_lp-1:0] alloc_idx;
  integer alloc_scan;
  ...
  logic base_wr_v;
  logic [bht_idx_width_p-1:0] base_wr_idx;
  logic [1:0] base_wr_data;

  logic tage_valid_wr_v;
  logic [provider_width_lp-1:0] tage_valid_wr_table;
  logic [tage_idx_width_lp-1:0] tage_valid_wr_idx;
  logic tage_valid_wr_data;
  ...
  logic provider_pred;
  logic [tage_idx_width_lp-1:0] provider_idx;
