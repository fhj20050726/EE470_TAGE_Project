...
`ifndef SYNTHESIS
  ...
  logic bp_fe_counters_en;
  longint unsigned bp_fe_counter_period;

  longint unsigned ctr_cycles;
  longint unsigned ctr_fetch_accept;
  longint unsigned ctr_icache_hit;
  longint unsigned ctr_icache_miss;

  longint unsigned ctr_btb_lookup;
  longint unsigned ctr_btb_hit;
  longint unsigned ctr_btb_pred_taken;

  longint unsigned ctr_cond_br_resolve;
  longint unsigned ctr_cond_br_correct;
  longint unsigned ctr_cond_br_mispredict;

  longint unsigned ctr_pred_taken_wrong;
  longint unsigned ctr_pred_ntaken_wrong;
  longint unsigned ctr_not_branch_wrong;

  longint unsigned ctr_bht_update;
  longint unsigned ctr_btb_update;

  initial begin
    bp_fe_counters_en = $test$plusargs("bp_fe_counters");
    if (!$value$plusargs("bp_fe_counters_period=%d", bp_fe_counter_period))
      bp_fe_counter_period = 64'd0;
  end

  wire fe_fetch_accept = fetch_yumi_i;

  wire cond_br_correct =
    attaboy_yumi_o
    & attaboy_br_metadata_fwd_cast_i.site_br;

  wire cond_br_mispredict =
    redirect_br_v_i
    & redirect_br_metadata_fwd_cast_i.site_br;

  wire cond_br_resolve =
    cond_br_correct | cond_br_mispredict;

  wire pred_taken_wrong =
    redirect_br_v_i
    & redirect_br_metadata_fwd_cast_i.site_br
    & redirect_br_ntaken_i;

  wire pred_ntaken_wrong =
    redirect_br_v_i
    & redirect_br_metadata_fwd_cast_i.site_br
    & redirect_br_taken_i;

  wire not_branch_wrong =
    redirect_br_v_i
    & redirect_br_nonbr_i;
  ...
  always_ff @(posedge clk_i) begin
    if (reset_i) begin
      ...
      ctr_btb_lookup         <= '0;
      ctr_btb_hit            <= '0;
      ctr_btb_pred_taken     <= '0;
      ctr_cond_br_resolve    <= '0;
      ctr_cond_br_correct    <= '0;
      ctr_cond_br_mispredict <= '0;
      ctr_pred_taken_wrong   <= '0;
      ctr_pred_ntaken_wrong  <= '0;
      ctr_not_branch_wrong   <= '0;
      ctr_bht_update         <= '0;
      ctr_btb_update         <= '0;
    end else begin
      ctr_cycles <= ctr_cycles + 1'b1;

      if (fe_fetch_accept)
        ctr_fetch_accept <= ctr_fetch_accept + 1'b1;

      if (icache_hit_v_i)
        ctr_icache_hit <= ctr_icache_hit + 1'b1;

      if (icache_miss_v_i)
        ctr_icache_miss <= ctr_icache_miss + 1'b1;

      if (bht_r_v_li) begin
        ctr_btb_lookup <= ctr_btb_lookup + 1'b1;

        if (btb_br_tgt_v_lo)
          ctr_btb_hit <= ctr_btb_hit + 1'b1;

        if (btb_taken)
          ctr_btb_pred_taken <= ctr_btb_pred_taken + 1'b1;
      end

      if (cond_br_resolve)
        ctr_cond_br_resolve <= ctr_cond_br_resolve + 1'b1;

      if (cond_br_correct)
        ctr_cond_br_correct <= ctr_cond_br_correct + 1'b1;

      if (cond_br_mispredict)
        ctr_cond_br_mispredict <= ctr_cond_br_mispredict + 1'b1;

      if (pred_taken_wrong)
        ctr_pred_taken_wrong <= ctr_pred_taken_wrong + 1'b1;

      if (pred_ntaken_wrong)
        ctr_pred_ntaken_wrong <= ctr_pred_ntaken_wrong + 1'b1;

      if (not_branch_wrong)
        ctr_not_branch_wrong <= ctr_not_branch_wrong + 1'b1;

      if (bht_w_v_li & bht_w_yumi_lo)
        ctr_bht_update <= ctr_bht_update + 1'b1;

      if (btb_w_v_li & btb_w_yumi_lo)
        ctr_btb_update <= ctr_btb_update + 1'b1;

      if (bp_fe_counters_en
          && (bp_fe_counter_period != 0)
          && ((ctr_cycles % bp_fe_counter_period) == 0)) begin
        $display("[BP_FE_COUNTERS] cycles=%0d fetch=%0d ic_hit=%0d ic_miss=%0d btb_lookup=%0d btb_hit=%0d btb_pred_taken=%0d br_resolve=%0d br_correct=%0d br_mispredict=%0d pred_taken_wrong=%0d pred_ntaken_wrong=%0d not_branch_wrong=%0d bht_update=%0d btb_update=%0d",
          ctr_cycles,
          ctr_fetch_accept,
          ctr_icache_hit,
          ctr_icache_miss,
          ...
          ctr_cond_br_resolve,
          ctr_cond_br_correct,
          ctr_cond_br_mispredict,
          ctr_pred_taken_wrong,
          ctr_pred_ntaken_wrong,
          ctr_not_branch_wrong,
          ctr_bht_update,
          ctr_btb_update
        );
      end
  ...
  final begin
    if (bp_fe_counters_en) begin
      $display("");
      $display("========== BP FE BASELINE COUNTERS ==========");
      $display("cycles              = %0d", ctr_cycles);
      $display("fetch_accept        = %0d", ctr_fetch_accept);
      $display("icache_hit          = %0d", ctr_icache_hit);
      $display("icache_miss         = %0d", ctr_icache_miss);
      $display("btb_lookup          = %0d", ctr_btb_lookup);
      $display("btb_hit             = %0d", ctr_btb_hit);
      $display("btb_pred_taken      = %0d", ctr_btb_pred_taken);
      $display("cond_br_resolve     = %0d", ctr_cond_br_resolve);
      $display("cond_br_correct     = %0d", ctr_cond_br_correct);
      $display("cond_br_mispredict  = %0d", ctr_cond_br_mispredict);
      $display("pred_taken_wrong    = %0d", ctr_pred_taken_wrong);
      $display("pred_ntaken_wrong   = %0d", ctr_pred_ntaken_wrong);
      $display("not_branch_wrong    = %0d", ctr_not_branch_wrong);
      $display("bht_update          = %0d", ctr_bht_update);
      $display("btb_update          = %0d", ctr_btb_update);

      if (ctr_cond_br_resolve != 0) begin
        $display("branch_accuracy_pct = %0f",
          100.0 * real'(ctr_cond_br_correct) / real'(ctr_cond_br_resolve));
        $display("branch_miss_pct     = %0f",
          100.0 * real'(ctr_cond_br_mispredict) / real'(ctr_cond_br_resolve));
      end

      if (ctr_btb_lookup != 0) begin
        $display("btb_hit_rate_pct    = %0f",
          100.0 * real'(ctr_btb_hit) / real'(ctr_btb_lookup));
      end
